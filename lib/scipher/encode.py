...
def main():
	...
	parser.add_argument(
		"--seed", metavar="S", type=int, help="the random number generator seed"
	)
	parser.add_argument(
	...
	parser.add_argument(
		...
		metavar="W",
		type=str,
		help="a website link to include, if any " '(must start with "https://")',
	)
	args = parser.parse_args()

	if args.socket:
		ok = scipherd.call_daemon(args.socket, True, args.infile, args.outfile)
		if ok:
			exit(0)
		else:
			exit(-1)
	...
	input_text = open(args.infile).read()
	if len(input_text) > 2 ** 20:
		raise Exception("Input text must be smaller than 1MB.")

	message = encode_text(input_text, args.website)

	print("================ START MESSAGE ================")
	print(message)
	print("================ END MESSAGE ================")
	if args.outfile:
		print("Outfile specified, writing to %s" % args.outfile)
		open(args.outfile, "w").write(message)
	else:
