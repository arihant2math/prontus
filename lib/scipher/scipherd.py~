...
class States:
    def __init__(self):
        ...
        while version >= 0:
            ...
            # encode state
            header_grammar = nltk.data.load(
                "file:%s" % common.header_cfg_filename(), "cfg"
            )
            body_grammar = nltk.data.load("file:%s" % common.body_cfg_filename(), "cfg")
            space_before = re.compile(
                "\s([%s])" % common.chars_to_remove_a_space_before()
            )
            space_after = re.compile(
                "([%s])\s" % common.chars_to_remove_a_space_after()
            )
            last_or_nots = common.choose_last_or_nots()
            estate = encode.EncodeState(
                "",
                None,
                common,
                header_grammar,
            ...
            de_body_grammar = decode.load_and_norm_grammar(common.body_cfg_filename())
