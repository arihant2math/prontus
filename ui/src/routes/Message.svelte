...
{#if !systemMessage}
    <div id="{messageId}" class="pl-5 py-2 flex items-start gap-2.5 hover:bg-gray-100 dark:hover:bg-slate-800" role="listitem">
        {#if !repeat}
...
<!--            <p class="text-sm font-normal py-2.5 text-gray-900 dark:text-white">{message.message}</p>-->
        ...
        <ul data-popover data-popover-target="{messageId}" data-popover-position="right" data-popover-offset="-300" data-popover-configure class="fixed flex flex-row text-sm bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 rounded-lg shadow-lg" id="{reactionsId}">
            <li>
...
<!--            TODO: Forward button -->
            <li>
                <button class="block w-full text-left px-2 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" data-popover-ref-target="reactions">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
