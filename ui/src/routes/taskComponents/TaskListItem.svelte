<script>
    import {completeTask, deleteTask, uncompleteTask} from "$lib/api.js";

    export let task;
</script>
{#if task.completed !== null && task.completed}
    <div id="task" class="flex justify-between items-center border-b border-slate-200 py-3 px-2 border-l-4 border-l-transparent w-full" onclick={() => {uncompleteTask(task.id)}}>
...
{:else}
    <div id="task" class="flex justify-between items-center border-b border-slate-200 py-3 px-2 border-l-4 border-l-transparent bg-gradient-to-r from-transparent to-transparent hover:from-slate-100 transition ease-linear duration-150 rounded-lg w-full" onclick={() => {completeTask(task.id)}}>
        <div class="inline-flex items-center space-x-2">
            <div>
